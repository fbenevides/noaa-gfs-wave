...
def format_changelog_section(version: str, on: date, commits: dict[str, list[str]]) -> str:
    parts = [_format_type_section(t, commits) for t in _TYPE_TO_SECTION]
    body = "".join(p for p in parts if p)
    return f"## [{version}] - {on.isoformat()}\n\n{body}"


def _format_type_section(ctype: str, commits: dict[str, list[str]]) -> str:
    entries = commits.get(ctype, [])
    if not entries:
        return ""
    lines = [f"### {_TYPE_TO_SECTION[ctype]}", ""] + [f"- {e}" for e in entries] + ["", ""]
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    section = format_changelog_section(new_version, date.today(), commits)
